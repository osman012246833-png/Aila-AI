...
# --- 1. الهوية والأيقونة (خط أحمر لا يمكن المساس به) ---
...
def speak(text):
    try:
        ...
        pass # عشان التطبيق ميعلقش لو فيه مشكلة في الصوت
...
# --- 4. تصميم الواجهة (نفس الشكل الرائع بدون تغيير) ---
st.markdown("""
    <style>
...
if not st.session_state.is_authenticated:
    ...
    if not st.session_state.welcome_sent:
        if st.session_state.is_maker:
            welcome_msg = f"تحياتي وإجلالي لك يا صانعي العظيم، الزعيم عثمان. عالم آيلا تحت أمرك، كيف أخدمك يا ملكي؟ الساعة الآن: {get_current_time()}"
        else:
            welcome_msg = f"أهلاً بك في عالم آيلا الذكي، {st.session_state.user_display_name}. كيف يمكنني مساعدتك؟ الساعة الآن: {get_current_time()}"
        
        st.session_state.messages.append({"role": "assistant", "content": welcome_msg})
        st.session_state.welcome_sent = True
        speak(welcome_msg) # آيلا تتحدث عند الدخول
    ...
    if prompt := st.chat_input("تحدثي معي يا آيلا..."):
        ...
        try:
            ...
            if "الساعة" in prompt or "الوقت" in prompt:
                answer = f"الساعة الآن في القاهرة يا زعيم هي {get_current_time()}."
            else:
                if st.session_state.is_maker:
                    sys_prompt = f"أنتِ آيلا AI. تتحدثين الآن مع صانعك ومبتكرك 'الزعيم عثمان'. يجب أن تتحدثي معه بمنتهى الحب والاحترام والتقدير والولاء. استخدمي كلمات مثل 'يا ملكي'، 'يا صانعي'، 'يا زعيم'. كوني ودودة جداً وغير رسمية معه."
                else:
            ...
            with st.chat_message("assistant"):
                ...
                speak(answer) # آيلا تنطق الرد فوراً
